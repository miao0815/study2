...
const STUDY_VERSION = "study2_turnaround_v2_teacher_fix_2026-06-16";
...
function makeOperationSequence(cond) {
  ...
  pushPhase(5, -40, 5);  // 中亏
  pushPhase(3, -70, 5);  // 大亏
  pushPhase(5, -40, 5);  // 回到中亏
  if (cond === "mid_loss_recovery") {
    pushPhase(10, -40, 5); // 最后 1/3 仍处于中亏
  } else if (cond === "small_loss_recovery") {
    pushPhase(10, -20, 5); // 最后 1/3 进入小亏
  } else {
    pushPhase(5, -20, 5);  // 接近盈亏平衡的小亏
    pushPhase(8, 40, 5);   // 稳定盈利
  }
...
function getRecentItems(historyArr, currentIndex, labelPrefix) {
  const items = [];
  if (currentIndex - 2 >= 0) {
    const prev2 = historyArr[currentIndex - 2];
    items.push(`${labelPrefix}${prev2.index}：${formatMoney(prev2.result)}`);
  }
  if (currentIndex - 1 >= 0) {
    const prev1 = historyArr[currentIndex - 1];
    items.push(`${labelPrefix}${prev1.index}：${formatMoney(prev1.result)}`);
  }
  return items;
}
...
const welcome = {
  ...
  stimulus: `
    <div class="card center">
      ...
      <div class="notice">本实验采用“基础参与奖励 + 表现相关奖励”的方式。表现相关奖励会参考你在任务中的最终公司账户表现进行核算，但不是账户金额与实际奖励的一一对应。</div>
...
function makeOperationMonthTrial(m){
  return {
    ...
    stimulus:function(){
      ...
      const recentItems = getRecentItems(operationHistory, operationHistory.length - 1, "第");
...
function makeStrategicRatingTrial(r){
  return {
    ...
    html:function(){
      ...
      const recentItems = getRecentItems(strategicHistory, strategicHistory.length, "第").map(t => t.replace("第", "第").replace("：", "轮："));
      return `
        <div class="card">
          <div class="tag">CEO战略项目评估</div>
          <div class="title">项目第 ${r.round} 个决策月</div>
          <div class="data-box">
            <div class="month-result-label">本月项目结果</div>
...
function makeStrategicDecisionTrial(r){
  return {
    ...
    stimulus:function(){
      ...
      const recentItems = getRecentItems(strategicHistory, strategicHistory.length - 1, "第").map(t => t.replace("第", "第").replace("：", "轮："));
      if (isFinal) {
        stop_point = r.round;
...
const save_data_trial = {
  ...
  filename: `study2__.csv`,
  data_string: ()=>jsPsych.data.get().csv(),
  on_finish:function(data){ data.phase = "save_data"; }
};
